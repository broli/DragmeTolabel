...
def test_write_label_file_atomically_replaces_existing_file(
    ...
    monkeypatch.setattr("labelme._fs.os.replace", _replace)
...
def test_write_label_file_close_failure_preserves_existing_file(
    ...
    def _fdopen_with_failing_close(fd: int, *args: object, **kwargs: object) -> TextIO:
        file = real_fdopen(fd, *args, **kwargs)  # ty: ignore[no-matching-overload]
        real_close = file.close

        def _fail_close() -> None:
            real_close()
            raise OSError("close failed")

        file.close = _fail_close
        return file

    monkeypatch.setattr("labelme._fs.os.fdopen", _fdopen_with_failing_close)
...
def test_write_label_file_replacement_failure_preserves_existing_file(
    ...
    monkeypatch.setattr("labelme._fs.os.replace", _fail_replace)
