...
- Fixed failed Annotation File saves truncating the last good file. Saves now write and close a temporary file in the target directory before replacing the destination, preserve the existing file's mode bits across the replacement, and remove the temporary file on any failure ([#2477](https://github.com/wkentaro/labelme/pull/2477))
